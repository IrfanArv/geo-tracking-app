...
const RealTimeMap = (): React.JSX.Element => {
  ...
  const startTracking = async () => {
    try {
      ...
      watchId.current = Geolocation.watchPosition(
        ...
        {
          enableHighAccuracy: true,
          distanceFilter: 1, // Update location every 100 meters
        },
      );
    } catch (error) {
  ...
  const stopTracking = async () => {
    try {
      ...
      const lastLocation =
        locations.length > 0
          ? locations[locations.length - 1] // Get the last location from the array
          : {latitude: 0, longitude: 0, reverseData: 'Unknown'}; // Default to 0,0 and Unknown reverseData

      // If reverseData is missing for the last location, fetch it
  ...
  return (
    <View style={styles.container}>
      ...
      {/* FlatList for displaying location updates */}
      <FlatList
        data={locations}
